...
class CountriesData(Dataframe):
    def process_data(self):
        countries_A = self.conjunto_A[['titular_pais_nacimiento', 'titular_pais_nacimiento_id']]
        countries_B = self.conjunto_B[['titular_pais_nacimiento', 'titular_pais_nacimiento_id']]
        ...
        countries_clean = countries_total[countries_total["code"].notna()] # Elimino registros que en la columna code sean NaN
        return countries_clean

class ProvincesData(Dataframe):
    def process_data(self):
        provinces_A = self.conjunto_A[['registro_seccional_provincia', 'titular_domicilio_provincia_id', 'titular_pais_nacimiento_id']]
        provinces_B = self.conjunto_B[['registro_seccional_provincia', 'titular_domicilio_provincia_id', 'titular_pais_nacimiento_id']]
...
class ProcedureData(Dataframe):
    def process_data(self):
        procedure_A = self.conjunto_A[['code_number', 'tramite_tipo', 'titular_domicilio_provincia_id']]
        procedure_B = self.conjunto_B[['code_number', 'tramite_tipo', 'titular_domicilio_provincia_id']]
